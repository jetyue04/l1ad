...
def main():
    ...
    parser.add_argument("--vicreg-checkpoint", required=True, help="Path to VICReg .pt checkpoint")
    parser.add_argument("--wnae-config",       default="config/config.yaml")
    parser.add_argument("--data",              default=None, help="Override data filepath")
    parser.add_argument("--outdir",            default="output_wnae_from_vicreg")
    parser.add_argument("--freeze-encoder",    action="store_true", help="Freeze encoder weights during WNAE training")
    ...
    vicreg_epoch = ckpt.get("epoch", "?")
    print(f"  VICReg epoch: {vicreg_epoch}")

    # Reconstruct encoder from config to load state dict cleanly
    ...
    # Pull encoder weights out of the full VICReg state dict
    ...
    print(f"  Encoder architecture: 57 → {enc_cfg['intermediate_architecture']} → {enc_cfg['bottleneck_size']}")
    ...
    # ── Build decoder to match encoder bottleneck ─────────────────────────────
    bottleneck = enc_cfg["bottleneck_size"]
    # Mirror the encoder architecture for the decoder
    decoder_arch = list(enc_cfg["intermediate_architecture"])
    decoder = M.Decoder(
        output_size=57,
        intermediate_architecture=decoder_arch,
        bottleneck_size=bottleneck,
        drop_out=None,
    ).to(device)
    print(f"  Decoder architecture: {bottleneck} → {decoder_arch} → 57")

    # ── Load WNAE config and data ─────────────────────────────────────────────
